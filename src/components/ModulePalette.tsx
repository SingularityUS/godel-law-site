...
import { MODULE_DEFINITIONS, ModuleDefinition } from "@/data/modules";
import { useModuleColors } from "@/hooks/useModuleColors";

interface ModulePaletteProps {
  onDragStart: (module: ModuleDefinition, event: React.DragEvent) => void;
}
  ...
  return (
    <div className="w-full flex gap-2 flex-wrap py-2 px-1 overflow-x-auto">
      {MODULE_DEFINITIONS.map((mod) => {
        const moduleColor = getModuleColor(mod.type);
        return (
          <div
            key={mod.type}
            className={`w-24 h-16 border-2 border-black cursor-grab select-none hover:shadow-lg transition-shadow ${moduleColor}`}
            draggable
