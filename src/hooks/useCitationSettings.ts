...
const DEFAULT_PROMPT = `You are a legal‐citation editor.
Your task:
1.	Read the full document text including the invisible anchor tags (⟦…⟧).
...
export const useCitationSettings = () => {
  const { user } = useAuth();
  const [settings, setSettings] = useState<CitationSettings | null>(null);
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      console.error('Error loading citation settings:', error);
      toast({
        title: "Error",
        description: "Failed to load citation settings",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const saveSettings = async (prompt: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Citation extraction prompt updated successfully",
      });
    } catch (error) {
      console.error('Error saving citation settings:', error);
      toast({
        title: "Save Failed",
        description: "Failed to save citation settings",
        variant: "destructive",
      });
      throw error;
    } finally {
  ...
  const resetToDefault = async () => {
    ...
    setIsSaving(true);
    try {
      await saveSettings(DEFAULT_PROMPT);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const getCurrentPrompt = () => {
    return settings?.prompt || DEFAULT_PROMPT;
  };
